            ...
            <div class="modal-body">
                ...
                <div id="abc" hidden class="row">
                        ...
                        <div class="col-sm-12 form-group">
                            ...
                            Greetings From Premier Real Estate.

                            With reference to your site visit on  assisted by Mr. abhishek from Premier Real Estate, we thank you for giving us an opportunity to serve you in searching your dream home.  At PMR it is our endeavour to help you with all the possible Property options which will suit your requirement. Mr.  from PMR will be at your service. He/she will be there to assist you in searching your dream home.
                            ...
                            6. Rental Services - Need to rent your house Or searching for good house on rent do not worry try Premier Real Estate rental services. Our professional and need based approach will ensure that you get right home/tenant without much hassles.
                        ...
                        <div id="clientEmail" hidden>
                            ...
                            <div class="col-sm-12 form-group">
                                ...
                                <textarea class="form-control" name="notes" id="client_email_body" rows="15" id="comment">
                                    ...
                                    Kindly register the below client For __________________ project On behalf Of Premier Real Estate 
    ...
    function edit(v){
        ...
        $.ajax({
            ...
            success:function(data){
                ...
                $('#c_seniorAdvisor').val(data.advisor1_id);
                $('#c_secondAdvisor').val(data.advisor2_id);
                $('#c_bkngName').val(data.booking);
                $('#c_bkngMnth').val(data.booking_month);
                $('#c_dateofClosure').val(data.closure_date);
                $('#c_customerName').val(data.customer);
                $('#c_projectName').val(data.project_id);
                $('#c_sqftSold').val(data.sqft_sold);
                $('#c_plcCharge').val(data.plc_charge);
                $('#c_floorRise').val(data.floor_rise);
                $('#c_basicCost').val(data.basic_cost);
                $('#c_otherCost').val(data.other_cost);
                $('#c_carPark').val(data.car_park);
                $('#c_totalCost').val(data.total_cost);
                $('#c_comission').val(data.commission);
                $('#c_grossRevenue').val(data.gross_revenue);
                $('#c_cashBack').val(data.cash_back);
                $('#c_subBrokerAmo').val(data.sub_broker_amo);
                $('#c_netRevenue').val(data.net_revenue);
                $('#c_shareAdvisor1').val(data.share_of_advisor1);
                $('#c_shareAdvisor2').val(data.share_of_advisor2);
                $('#c_estMonthofInvoice').val(data.est_month_of_invoice);
                $('#c_agrmntStatus').val(data.agreement_status);
                $('#c_projectType').val(data.project_type);
                $('#c_subSource').val(data.sub_source_id);
