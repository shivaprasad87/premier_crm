					...
					foreach ($advisors as $key => $value) { 
						...
						?>
					 	<tr>
					 		...
					 		<td>
					 			<?php if($value > 0){ ?>
					 				<a href="<?php echo base_url().'view_callbacks?report=clent_reg&advisor='.urlencode($key).'&dept='.urlencode($dept).'&city='.urlencode($city).'&fromDate='.urlencode($fromDate).'&toDate='.urlencode($toDate); ?>"><?php echo $value['count']; ?></a>
					 			<?php }else{
					 				echo $value['count'];
					 			} ?>
					 		</td>
